        ...
        <v-col>
            ...
            <template v-slot:top>
                <v-toolbar flat color="white">
                    ...
                    <v-toolbar-title>Cuenta Contable</v-toolbar-title>
                    <v-snackbar
                        v-model="snackbar"
                        :timeout="timeout"
                        right
                            ...
                            <v-container grid-list-md>
                                <v-row dense>
                                    ...
                                    <v-col cols="12" sm="3" md="3">
                                        <v-text-field v-model="apporigen" label="Origen"></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="3" md="3">
  ...
  export default {
    data: () => ({
        ...
        headers: [
            ...
            { text: 'Origen', value: 'apporigen', align: 'start', sortable: true },
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Nueva Cuenta Contable' : 'Actualizar Cuenta Contable'
      },
    },
    ...
    methods: {
        crearPDF(){
            var columns = [
                ...
                {title: "Origen", dataKey: "apporigen"},
            ...
            this.concuentas.map(function(x){
                rows.push({empresa:x.empresa, apporigen:x.apporigen,moneda:x.nomneda,numcuenta:x.numcuenta,activo:x.activo});
            });
            ...
            doc.autoTable(columns, rows, {
                ...
                addPageContent: function(data) {
                    doc.text("Listado de Concuentas", 40, 30);
                }
            });
            doc.save('Concuentas.pdf');
        },
        listar(){
            let me=this;
            ...
            axios.get('api/Concuentas/Listar',configuracion).then(function(response){
                // console.log(response);
                me.concuentas=response.data;
        ...
        deleteItem (item) {
            ...
            if (resulta) {
                ...
                let configuracion= {headers : header};
                axios.delete('api/Concuentas/Eliminar/'+item.id,configuracion).then(function(response){
                    me.close();
                    me.listar();
                }).catch(function(error){
                    me.snacktext = 'Se detectó un error. Código: '+ error.response.status;
        ...
        guardar () {
            ...
            if (me.editedIndex > -1) {
                //Código para editar
                //Código para guardar
                axios.put('api/Concuentas/Actualizar',{
                    'Id':me.id,
                    'empresaId': me.empresaId,
                    'apporigen': me.apporigen,
                    'moneda': me.moneda,
                ...
                //Código para guardar
                axios.post('api/Concuentas/Crear',{
                    'empresaId': me.empresaId,
                    'apporigen': me.apporigen,
                    'moneda': me.moneda,
                    'numcuenta': me.numcuenta,
        ...
        activar(){
            ...
            let header={"Authorization" : "Bearer " + this.$store.state.token};
            let configuracion= {headers : header};
            axios.put('api/Concuentas/Activar/'+this.adId,{},configuracion).then(function(response){
                me.adModal=0;
                me.adAccion=0;
                me.adNombre="";
                me.adId="";
        ...
        desactivar(){
            ...
            let header={"Authorization" : "Bearer " + this.$store.state.token};
            let configuracion= {headers : header};
            axios.put('api/Concuentas/Desactivar/'+this.adId,{},configuracion).then(function(response){
                me.adModal=0;
                me.adAccion=0;
                me.adNombre="";
                me.adId="";
